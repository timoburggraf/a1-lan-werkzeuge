#!/usr/bin/env bash
# Startet die Standby-Sperre mit den Druckerdaten aus dem Vault.
# Braucht keinen API-Schluessel — dieser Dienst fragt nur den Drucker.
set -euo pipefail
...
exec "$BASIS/.venv/bin/python" "$BASIS/standby_sperre.py"
